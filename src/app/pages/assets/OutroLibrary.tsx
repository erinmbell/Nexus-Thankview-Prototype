    ...
    <div className="p-4 md:p-8 pt-0 min-h-full">
      ...
      <div className="flex flex-col gap-3 mb-6">
        ...
        <div className="flex items-center gap-2 bg-white rounded-full px-4 py-1.5 border border-tv-border-light min-w-[200px] w-[255px] shrink-0">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search outros..." aria-label="Search outros" className="bg-transparent text-[13px] text-tv-text-primary outline-none w-full placeholder:text-tv-text-decorative focus-visible:outline-none" />
          {search && (
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map(outro => (
            <div role="button" tabIndex={0} key={outro.id} onClick={() => setPreviewId(outro.id)} onKeyDown={e => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setPreviewId(outro.id); }}} aria-label={`Preview: ${outro.name}`} className="bg-white rounded-xl border border-tv-border-light overflow-hidden hover:shadow-md hover:border-tv-border-strong transition-all group cursor-pointer text-left">
              {/* Thumbnail */}
              ...
              <div className="p-3.5">
                <div className="flex items-start justify-between mb-2">
                  ...
                  <div className="relative" onClick={e => e.stopPropagation()}>
                    <button onClick={() => setMenuOpen(menuOpen === outro.id ? null : outro.id)} aria-label="More actions" aria-haspopup="menu" aria-expanded={menuOpen === outro.id} className="p-1 rounded hover:bg-tv-surface transition-colors">
