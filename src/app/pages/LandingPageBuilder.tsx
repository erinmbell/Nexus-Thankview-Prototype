                        ...
                        <div className="space-y-3">
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            ...
                            {backgrounds.filter(bg => bg.kind === "image").map(bg => (
                              <div role="button" tabIndex={0} key={bg.id} className={`group relative rounded-md overflow-hidden border-2 transition-all cursor-pointer text-left ${selectedBgId === bg.id ? "border-tv-brand ring-2 ring-tv-brand/20" : "border-tv-border-light hover:border-tv-border-strong"}`} onClick={() => trackedSetSelectedBgId(bg.id)} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); trackedSetSelectedBgId(bg.id); } }}>
