...
import {
  ...
  Undo2, Redo2, Plus, Heart, Shield, Building2, Award,
  RotateCcw, Upload,
} from "lucide-react";
...
export function EnvelopeBuilder() {
  ...
  // postmarkColor removed — postmark is now simply black or white
  ...
  // (Recently-used logo resolution handled inside EnvelopePreview)
    ...
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <div className="flex-1 flex min-h-0">
        {wizardStep === "build" ? (
          <>
            {/* ── Left panel ──────────────────────────────────────────────── */}
            <div className="w-[320px] shrink-0 flex flex-col border-r border-tv-border-divider">
              {/* Tabs */}
                ...
                {activeTab === "marks" && (
                  <div className="space-y-5">
                    ...
                    <div className="pt-2 border-t border-tv-border-divider">
                      ...
                      <p className="text-[11px] text-tv-text-secondary mb-3">Include a stamp in .jpeg or .png format that can add personality or emphasize the message you want to share.</p>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {STAMP_OPTIONS.map(opt => {
                          ...
                          return (
                            <button key={opt.id} onClick={() => setStampStyle(opt.id)}
                              className={`flex flex-col items-center gap-1.5 px-3 py-3 rounded-[12px] border transition-all ${
                                selected
                                  ? "border-tv-brand-bg bg-tv-brand-tint"
                                  : "border-tv-border-light hover:border-tv-border-strong"
                              }`}>
                              <Icon size={18} className={selected ? "text-tv-brand" : "text-tv-text-secondary"} />
                              <span className={`text-[11px] ${selected ? "text-tv-brand" : "text-tv-text-secondary"}`}
                                style={{ fontWeight: selected ? 600 : 400 }}>{opt.label}</span>
                              {selected && <Check size={11} className="text-tv-brand" />}
                            </button>
                          );
                        })}
                      </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {(["front", "back", "open"] as const).map(v => (
                    <div key={v} className="rounded-[10px] overflow-hidden border border-tv-border-light shadow-sm">
                      <EnvelopePreview
                        view={v} envelopeColor={envelopeColor} nameColor={nameColor}
                        primaryColor={primaryColor} design={design}
                        swoop1Color={swoop1Color} swoop2Color={swoop2Color}
                        stripe1Color={stripe1Color} stripe2Color={stripe2Color}
                        postmark={postmark} postmarkText={postmarkText}
                        stampPreview={stampImagePreview}
                        logoPreview={frontLogo !== "none" ? (logoPreview || "placeholder") : null}
                        backFlapLogoPreview={backLogo !== "none" ? (backFlapLogoPreview || "placeholder") : null}
                        linerColor={linerColor}
      ...
      <LivePreviewModal
        ...
        postmarkText={postmarkText}
        stampPreview={stampImagePreview}
        logoPreview={frontLogo !== "none" ? logoPreview : null}
        backFlapLogoPreview={backLogo !== "none" ? backFlapLogoPreview : null}
        stampStyle={stampStyle}
      />
    </div>
  );
...
function PerforatedStamp({ size = 48, accentColor = "#c09696", stampStyle = "classic", customImage }: { size?: number; accentColor?: string; stampStyle?: StampStyle; customImage?: string | null }) {
  ...
  // Render stamp icon based on stampStyle
  const renderStampIcon = () => {
    const r = iconSize * 0.58;
    switch (stampStyle) {
      case "heart":
        // Heart shape centered in stamp
        return (
      ...
      case "forever":
        // Infinity / forever symbol with circle
        return (
          <>
      ...
      case "classic":
      default:
        // Classic stamp: building/pillar icon with circle
        return (
          <>
            <circle cx={cx} cy={cy} r={r} fill="none" stroke={accentColor} strokeWidth={1.2} />
            {/* Simple pillared building icon */}
  ...
  return (
    <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`} fill="none">
      ...
      {customImage ? (
        <image href={customImage} x={0} y={0} width={w} height={h} preserveAspectRatio="xMidYMid slice" clipPath={`url(#${clipId})`} />
      ) : (
        renderStampIcon()
      )}
    </svg>
    ...
    return (
      <div className="aspect-[3/2] rounded-[12px] overflow-hidden relative"
        ...
        <svg className="absolute inset-0 w-full h-full pointer-events-none" viewBox="0 0 400 266" preserveAspectRatio="none">
          ...
          {/* Bottom flap — tucked underneath, barely visible */}
          ...
          {/* Top flap — main closure, folded down over everything */}
          ...
          {/* Highlight along inner edge of top flap */}
        ...
        {/* Back flap logo — centered on the top flap */}
        {backFlapLogoPreview && (
        ...
        {/* Wax seal — at the flap intersection point */}
